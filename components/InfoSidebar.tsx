...
export const InfoSidebar: React.FC<InfoSidebarProps> = ({ isOpen, onClose, data, onItemClick, destination }) => {
  ...
  return (
    <>
      ...
      />
      <aside 
        className={`fixed top-0 right-0 h-full shadow-2xl z-40 transition-transform duration-500 ease-in-out w-full md:w-1/2 lg:w-2/5 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        style={{ backgroundImage: `url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
      >
        <div className="absolute inset-0 bg-gray-900/70 backdrop-blur-sm z-0" />
